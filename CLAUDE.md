...
2. **User data persists to SQLite through the API.** Never `localStorage` or `sessionStorage` for widget layouts, folders, canvas contents, project settings or anything else the user created. The only permitted uses are the existing ones: the theme preference in `theme.js`, the graph→connections handoff in `graph-common.js`, and the API bearer token on the phone (`schedule/api-auth.js`) — a credential, not user-created data.
3. **Relative fetch paths only.** `fetch("/api/...")`. No absolute URLs, no hard-coded ports, no `http://` literals in client code. Only `app.py` knows the port. This keeps a desktop webview build viable later. The phone day view (`/day`) still obeys this: `schedule/api-auth.js` wraps `window.fetch` to attach the bearer token to same-origin `/api/` calls, so client code is unchanged and still names no host or port. The token is only required when `config.ARCHIVE_HOST` is set to a non-loopback address (off by default); `app.py`'s `_guard_api_when_exposed` then checks every non-loopback `/api/` request against it, reusing the capture API's `_capture_auth_ok`.
